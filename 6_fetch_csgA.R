...
for(i in seq(1, file_len)){
  ...
  # b <- str_extract_all(gbff, pattern ="/gene=\"csgC\"")
  
  readlines_nr <- grep(pattern = "/gene=\"csgC\"", gbff[1:length(gbff)], ignore.case = T)
  readlines_nr
  readlines_nr_duplikat <- grep(pattern = "/gene=\"csgC\"", gbff[1:length(gbff)], ignore.case = T)
  ...
  if(length(lokacja_genu) != 0){
    ...
    f
    file.path <- "/home/jarek/extracting_sequences/csgC"
    ...
    for(n in seq(1, nrow(f))){
      if(f[n,1] == "complement"){
        ...
        setwd("/home/jarek/extracting_sequences/csgC");
        # if(length(readlines_nr) != length(readlines_nr_duplikat)){
        write.fasta(a, paste0(org_name, ", ",  t[n]), file.out = paste0(org_name, "_",  "csgC", ".fasta"), open = "a", nbchar = 60, as.string = FALSE)
        # }
        # else{
